...
Both ways a case can reach the fraud team - the agent escalating on its own (possibly after consulting the analyst), or the interrupt firing - flow through a plain merging gateway (`Escalated (either path)`) directly into `Freeze card`, which computes one consistent handoff explanation from whichever variables are actually set (there's no separate "prepare summary" step - it's just an input/output mapping on the same task) before the human handoff.
